...
int a[MAXN];
int st[MAXN * 4][41];
void build(int id , int type , int l , int r){
    if(l == r){
        st[id][type] = (a[l] == type);
        return ;
    }
    int mid = l + r;
    mid >>= 1;
    build(id << 1  , type , l , mid);
    build(id << 1 | 1 , type , mid + 1 , r);
...
int get(int id , int type , int l , int r , int u , int v){
    if(u > r || l > v){
        return 0;
    }
    if(u <= l && r <= v){
...
signed main(){
    ...
    for(int i = 1 ; i <= m ; i ++){
        cin >> type >> u >> v;
        if(type - 1){
            update(1 , a[u] , 1 , n , u , -1);
            update(1 , v , 1 , n , u , 1);
            a[u] = v;
        }else{
            res = 0;
            for(int j = 1 ; j <= 40 ; j ++){
                res += get(1 , j , 1 ,  n , u , v) >= 1;
            }
            cout << res << '\n';
        }
    }
